...
{
    char * enc1=strstr(path,encv1);
    if(enc1 != NULL) decipher(enc1);
	int res;
    char fpath[1000];
    char s[1000];
    sprintf(s, "%s", path);
    ...
    else sprintf(fpath, "%s%s",dirpath,s);
